...
- Inline links: when a neuron references a concept defined in another neuron,
  use a markdown link to it. Example: if a project neuron mentions "SIT" and
  `infrastructure/environments.md` defines SIT, write
  `[SIT](../../infrastructure/environments.md)` instead of plain "SIT".
  Before writing a neuron, scan its content for terms that match existing
  neuron titles, glossary entries, or lobe names, and link them.
